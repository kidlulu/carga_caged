...
library(RODBC)
library(tidyverse)
download.file('ftp://ftp.mtps.gov.br/pdet/microdados/NOVO%20CAGED/2020/202001/CAGEDMOV202001.7z','mov.7z', mode="wb")
system('"c:/Program Files/7-Zip/7z.exe" e -y mov.7z')
mov <- data.table::fread(file = 'CAGEDMOV202001.txt',
                         encoding = if_else(readr::guess_encoding('CAGEDMOV202001.txt')[[1,1]]=="ISO-8859-1",'Latin-1','UTF-8')) %>% 
...
for (y in 2020:2022){
  ...
  for (m in 1:12){
    ...
    download.file(url = if_else(m<10,
                                paste0(ftp,'CAGEDMOV',y,'0',m,'.7z'),
                                paste0(ftp,'CAGEDMOV',y,m,'.7z')),
                  destfile = 'mov.7z',
                  mode = "wb")
    system('"c:/Program Files/7-Zip/7z.exe" e -y mov.7z')
    mov <- data.table::fread(file = if_else(m<10,
                                            paste0('CAGEDMOV',y,'0',m,'.txt'),
                                            paste0('CAGEDMOV',y,m,'.txt')),
                             encoding = if_else(readr::guess_encoding(if_else(m<10,
                                                                              paste0('CAGEDMOV',y,'0',m,'.txt'),
                                                                              paste0('CAGEDMOV',y,m,'.txt')))[[1,1]]=="ISO-8859-1",'Latin-1','UTF-8')) %>% 
      dplyr::rename_all(list(~c("competencia","regiao","uf","municipio","secao","subclasse","saldo","cbo2002ocupacao","categoria","graudeinstrucao","idade",
                                "horascontratuais","racacor","sexo","tipoempregador","tipoestabelecimento","tipomovimentacao","tipodedeficiencia","indtrabintermitente",
                                "indtrabparcial","salario","tamestabjan","indicadoraprendiz","origemdainformacao","competenciadec","indicadordeforadoprazo",
                                "unidadesalariocodigo","valorsalariofixo")))
